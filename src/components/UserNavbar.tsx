...
export function UserNavbar({
  ...
  cartCount,
  onCartClick,
}: { logo?: React.ReactNode; cartCount: number; onCartClick?: () => void }) {
  const { user } = useUser();
  const { points, refreshPoints } = useLoyaltyPoints();
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // لم يعد هناك داعي لجلب النقاط هنا، يتم ذلك من خلال LoyaltyPointsContext
  }, [user?.id]);
      ...
      <div className="flex flex-1 flex-row items-center gap-4 w-full justify-between sm:justify-end">
        ...
        <div className="flex-shrink-0 flex items-center justify-center">
          <span className="bg-gradient-to-br from-yellow-400 via-yellow-200 to-yellow-50 text-yellow-900 dark:from-yellow-700 dark:via-yellow-600 dark:to-yellow-400 font-bold rounded-full px-6 py-2 shadow-lg border-2 border-yellow-300 dark:border-yellow-700 text-lg flex items-center gap-1 min-w-[48px] min-h-[48px] justify-center backdrop-blur-md">
            ...
            <span className="text-xs font-semibold ml-0.5 hidden sm:inline">نقطة</span>
          </span>
        </div>
        {/* Profile Avatar & Popover */}
        <Popover>
          <PopoverTrigger asChild>
            <button className="focus:outline-none rounded-full border-2 border-gold/60 shadow-xl hover:scale-110 transition-transform bg-white/90 dark:bg-kian-charcoal/90 border-yellow-200 dark:border-yellow-700" style={{ minWidth: 48, minHeight: 48 }}>
              <img
                src={user?.imageUrl}
                alt="profile"
          ...
          <PopoverContent className="w-80 p-7 rounded-3xl shadow-2xl bg-white/95 dark:bg-kian-charcoal/95 border-0 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-kian-burgundy dark:text-gold mb-1">{user?.fullName || user?.username || "ضيف"}</div>
              <div className="text-sm text-gray-500 dark:text-gray-300">{user?.primaryEmailAddress?.emailAddress || ""}</div>
            </div>
            <div className="mt-2 flex flex-col items-center gap-1">
              <span className="text-base font-semibold text-yellow-700 dark:text-yellow-300">عدد نقاط الولاء:</span>
              <span className="text-3xl font-extrabold bg-gradient-to-br from-yellow-400 via-yellow-200 to-yellow-50 text-yellow-900 dark:from-yellow-700 dark:via-yellow-600 dark:to-yellow-400 dark:text-yellow-100 rounded-full px-8 py-3 shadow border-2 border-yellow-300 dark:border-yellow-700 ">
                {loading ? <Loader2 className=" h-6 w-6" /> : points ?? "-"}
              </span>
            </div>
