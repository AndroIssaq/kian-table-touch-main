...
const RequestTimer = ({ createdAt, status, finishedAt }: { createdAt: string; status: string; finishedAt?: string }) => {
  ...
  useEffect(() => {
    ...
    if (status !== "completed") {
      intervalRef.current = setInterval(() => setSeconds(getElapsedSeconds()), 1000);
  ...
  return (
    <span className="ml-2 flex items-center gap-1 text-xs font-mono px-2 py-1 rounded-full bg-gradient-to-r from-yellow-200 via-yellow-100 to-yellow-50 dark:from-yellow-900 dark:via-yellow-800 dark:to-yellow-700 shadow-sm border border-yellow-300 dark:border-yellow-800">
      ...
      {status === "completed" && <span className="ml-1 text-green-600 dark:text-green-300 font-bold">✓</span>}
    </span>
  );
};
...
const RequestCard = ({
  ...
  <div className="bg-kian-sand dark:bg-kian-charcoal rounded-lg shadow p-3 sm:p-4 border border-kian-burgundy dark:border-kian-sand">
    ...
    <div className="flex flex-wrap gap-2 mt-3 justify-center">
      {colKey === "pending" && (
        <>
          <Button variant="outline" size="sm" className="bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800 dark:hover:bg-blue-900/40 px-2 py-0 h-7 rounded-full font-bold text-xs shadow border border-blue-300 dark:border-blue-700" onClick={() => onMarkAsNew(request.id)}>
            إعادة كجديد
          </Button>
          <Button variant="outline" size="sm" className="bg-green-400 text-green-900 border-green-200 hover:bg-green-500 dark:bg-green-900/40 dark:text-green-400 dark:border-green-800 dark:hover:bg-green-900/40 px-2 py-0 h-7 rounded-full font-bold text-xs shadow border border-green-300 dark:border-green-700" onClick={() => onMarkAsCompleted(request.id)}>
            {t("markComplete")}
          </Button>
        </>
      )}
      {colKey === "new" && (
        <Button variant="outline" size="sm" className="bg-yellow-50 text-yellow-700 border-yellow-200 hover:bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 dark:border-yellow-800 dark:hover:bg-yellow-900/40 px-2 py-0 h-7 rounded-full font-bold text-xs shadow border border-yellow-300 dark:border-yellow-700" onClick={() => onMarkAsPending(request.id)}>
          {t("markPending")}
        </Button>
      )}
      {colKey === "completed" && (
      ...
      <Button variant="outline" size="sm" className="bg-red-50 text-red-700 border-red-200 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800 dark:hover:bg-red-900/40 px-2 py-0 h-7 rounded-full font-bold text-xs shadow border border-red-300 dark:border-red-700" onClick={() => onDelete(request.id)}>
        {t("delete") || "حذف"}
      </Button>
    </div>
  </div>
...
const KanbanColumn = ({
  ...
  let headerColor = "";
  if (col.key === "new") headerColor = "bg-red-100 text-red-700 border border-red-300";
  else if (col.key === "pending") headerColor = "bg-yellow-100 text-yellow-800 border border-yellow-300";
  else if (col.key === "completed") headerColor = "bg-green-100 text-green-800 border border-green-300";
  return (
...
const StaffDashboard = () => {
  ...
  const requestsByStatus: Record<string, WaiterRequest[]> = { new: [], pending: [], completed: [] };
  ...
  const updateRequestStatus = async (id: string, status: string, extra: any = {}) => {
    try {
      ...
      const { error } = await supabase.from("waiter_requests").update(updateObj).eq("id", id);
      if (error) throw error;
      setRequests((prev) => prev.map((req) => req.id === id ? { ...req, ...updateObj } : req));
      let desc = "";
  ...
  const deleteRequest = async (id: string) => {
    try {
      const { error } = await supabase.from("waiter_requests").update({ deleted: true as any }).eq("id", id);
      if (error) throw error;
      setRequests((prev) => prev.filter((req) => req.id !== id));
      toast({ title: t("success"), description: t("deleted") });
    } catch (error) {
      toast({ title: t("failedToDeleteRequest"), description: t("failedToDeleteRequest"), variant: "destructive" });
    }
  };
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const { data, error } = await supabase.from("waiter_requests").select("*").eq("deleted", false).order("created_at", { ascending: false });
        if (error) throw error;
        if (data) {
          const normalized = data.map(req => ({ ...req, user_id: req.user_id || "" }));
          ...
          if (userIds.length > 0) {
            ...
            const { data: loyaltyData } = await supabase.from("loyalty_visits").select("user_id, points, point_status").in("user_id", userIds);
            if (loyaltyData && Array.isArray(loyaltyData)) {
              loyaltyData.forEach(item => {
                if ('user_id' in item) loyaltyInfoObj[item.user_id] = item;
              });
            }
          }
        ...
        toast({ title: t("error"), description: t("failedToLoadRequests"), variant: "destructive" });
      }
    };
    fetchRequests();
  ...
  return (
    <PageTransition>
      <div className="w-[90%] mx-auto mt-[20px]">
        <div className="mb-6"><ThemeLanguageToggle /></div>
        <div className="flex flex-col items-center mb-8 relative">
          <div className="flex items-center w-full justify-between">
            <h1 className="text-2xl md:text-3xl font-bold mb-2 flex items-center">{t("staffDashboard")}</h1>
        ...
        {requests.length === 0 ? (
          <div className="text-center py-12 px-4 text-[25px] border-2 border-dashed border-kian-sand dark:border-kian-burgundy rounded-xl">
            ...
            <p className="text-base sm:text-lg text-kian-charcoal/70 dark:text-kian-sand/70">{t("noRequests")}</p>
          </div>
        ) : (
          <DragDropContext onDragEnd={({ source, destination, draggableId }: DropResult) => {
            if (!destination) return;
            const sourceCol = source.droppableId;
            const destCol = destination.droppableId;
            if (sourceCol === destCol) return;
            setRequests((prev) => prev.map((req) => req.id === draggableId ? { ...req, status: destCol } : req));
            if (destCol === "completed") updateRequestStatus(draggableId, "completed");
            else if (destCol === "pending") updateRequestStatus(draggableId, "pending");
            else if (destCol === "new") updateRequestStatus(draggableId, "new");
          }}>
            <div className="flex flex-nowrap sm:flex-wrap gap-4 sm:gap-3 md:gap-6 overflow-x-auto sm:overflow-x-visible pb-4 justify-center w-full max-w-none">
              {kanbanColumns.map((col) => (
                <KanbanColumn key={col.key} col={col} requests={requestsByStatus[col.key]}>
                  {(request, idx) => (
                      ...
                      {(provided, snapshot) => (
                        <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                          <RequestCard
                            ...
                            onMarkAsCompleted={(id) => updateRequestStatus(id, "completed")}
                            onMarkAsPending={(id) => updateRequestStatus(id, "pending")}
                            onMarkAsNew={(id) => updateRequestStatus(id, "new")}
