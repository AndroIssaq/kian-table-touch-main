...
export default function DrinkRecomaindationSystem() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      `https://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${encodeURIComponent(LOCATION)}&lang=ar`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: catsRaw } = await supabase.from("categories" as any).select("id, name_ar, name_en");
      const cats = Array.isArray(catsRaw) ? catsRaw : [];
      setCategories(cats);
      // استخراج id للتصنيفات المطلوبة مع حماية ضد الأخطاء
      const getId = (cat: any) => (cat && typeof cat === 'object' && 'id' in cat ? cat.id : undefined);
      const hotCat = cats.find((c: any) => c.name_ar === "مشروبات ساخنة" || c.name_en === "Hot Drinks");
      const freshCat = cats.find((c: any) => c.name_ar === "مشروبات فريش" || c.name_en === "Fresh Drinks");
      const smoothieCat = cats.find((c: any) => c.name_ar === "سموزي" || c.name_en === "Smoothies");
      const ids = [getId(hotCat), getId(freshCat), getId(smoothieCat)].filter(Boolean);
      if (!ids.length) return;
      const { data: items } = await supabase.from("items").select("id, name_ar, name_en, category_id, price, points");
      setDrinks((items || []).filter((item: any) => ids.includes(item.category_id)));
    };
    fetchData();
  }, []);
  ...
  if (drinkType === "hot") {
    ...
    recommendedDrinks = drinks.filter((d) => {
      ...
      return cat && (cat.name_ar === "مشروبات ساخنة" || cat.name_en === "Hot Drinks");
    });
    ...
    recommendedDrinks = drinks.filter((d) => {
      const cat = categories.find((c) => c.id === d.category_id);
      return cat && (
        cat.name_ar === "مشروبات فريش" || cat.name_en === "Fresh Drinks" ||
        cat.name_ar === "سموزي" || cat.name_en === "Smoothies"
      );
    });
  ...
  // جمل التوصية
  const recommendationText = temp !== null
    ? drinkType === "hot"
      ? { emoji: "☕🍵", title: "جرب مشروب دافي!", desc: "الجو لطيف أو بارد، المشروبات الساخنة هتدفيك أكتر." }
      : drinkType === "cold"
        ? { emoji: "🧊🥤", title: "جرب مشروب مثلج أو عصير بارد!", desc: "الجو حر اليوم، المشروبات الباردة هتنعشك!" }
        : { emoji: "🥤☕", title: "مشروبات باردة أو ساخنة؟", desc: "الجو معتدل، جرب اللي نفسك فيه!" }
    : null;
  ...
  return (
    <Card className="w-full flex flex-col items-center gap-3 p-4 sm:p-5 rounded-2xl shadow-lg bg-gradient-to-r from-[#f9f6ff] to-[#fff8f0] dark:from-[#23243a] dark:to-[#181a20] border-2 border-gold/10 my-4">
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-yellow-700 font-bold"><Loader2 className="animate-spin" /> جاري جلب حالة الطقس...</div>
      ) : error ? (
        <div className="text-red-600 font-bold">{error}</div>
      ) : recommendationText ? (
        <div className="w-full flex flex-col items-center gap-2">
          <div className="text-3xl mb-1">{recommendationText.emoji}</div>
          <div className="font-extrabold text-lg text-kian-burgundy dark:text-gold text-center mb-1">
            درجة الحرارة الآن: <span className="text-yellow-700 dark:text-yellow-200">{temp}°C</span>
          </div>
          <div className="text-base text-gray-700 dark:text-yellow-100 text-center mb-2">
            ...
            <span className="font-bold text-yellow-900 dark:text-yellow-200">{recommendationText.title}</span>
          </div>
          <div className="text-sm text-gray-600 dark:text-yellow-200 text-center mb-2">{recommendationText.desc}</div>
          <div className="flex flex-wrap gap-2 justify-center mt-2">
            {recommendedDrinks.length === 0 ? (
              <span className="text-gray-400">لا توجد مشروبات متاحة حالياً</span>
            ) : (
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent className="max-w-sm w-full rounded-2xl shadow-2xl">
          ...
          <div className="text-center text-base mb-4">
            {selectedDrink && (
              <>
                هل أنت متأكد أنك تريد طلب "{selectedDrink.name_ar}"؟ سيتم إرسال الطلب للنادل.
              </>
            )}
          </div>
