...
export default function Reports() {
    ...
    useEffect(() => {
        async function fetchItems() {
            const { data } = await supabase
                .from('items')
                .select('name_ar, name_en, price, points')
            setItems(data || [])
        }
        fetchItems()
    }, [])
    ...
    waiterRequests.forEach((r) => {
        ...
        if (item) {
            ...
            if (!productOrderMap[key])
                productOrderMap[key] = { name: item.name_ar, count: 0 }
    ...
    waiterRequests.forEach((r) => {
        ...
        if (item) {
            ...
            if (!productRevenueMap[key])
                productRevenueMap[key] = { name: item.name_ar, revenue: 0 }
    ...
    const uniqueCustomers = new Set(loyaltyVisits.map((v) => v.phone_number))
        .size
    // 2. أفضل العملاء
    ...
    const rewardStats = {
        free_drink: loyaltyVisits.filter((v) => v.points >= 10 && v.points < 20)
            .length,
    ...
    const loyaltyByDayArr = Object.entries(loyaltyByDay).map(
    ...
    const requestsByTableArr = Object.entries(requestsByTable).map(
    ...
    const avgResponse = (() => {
        ...
        const totalSeconds = completed.reduce((sum, r) => {
            if (
                typeof r.response_time === 'number' &&
                !isNaN(r.response_time)
            ) {
    ...
    const topRequests = (() => {
        ...
        waiterRequests.forEach((r) => {
            if (r.request && r.request !== 'لا يوجد طلب محدد')
                map[r.request] = (map[r.request] || 0) + 1
        })
    ...
    const peak = requestsByHour.reduce(
    ...
    const totalPoints = loyaltyVisits.reduce(
        (sum, v) => sum + (v.points || 0),
        0
    )

    // 3. عدد المشتريات بنقاط الولاء
    const loyaltyPurchases = waiterRequests.filter(
        (r) => r.request && r.request.includes('نقاط الولاء')
    ).length

    // 4. عدد المشتريات النقدية
    const cashPurchases = waiterRequests.filter(
        (r) => r.request && !r.request.includes('نقاط الولاء')
    ).length
    ...
    const retainedCustomersCount = loyaltyVisits.filter(
        (v) => v.points > 1
    ).length
    const retentionRate =
        uniqueCustomers > 0
            ? ((retainedCustomersCount / uniqueCustomers) * 100).toFixed(1)
            : 0
    ...
    const avgOrderValue =
        orderCountWithPrice > 0
            ? (totalRevenue / orderCountWithPrice).toFixed(2)
            : '0'
    ...
    waiterRequests.forEach((r) => {
        if (!r.user_name || !r.request || r.request === 'لا يوجد طلب محدد')
            return
        const item = getItemByRequest(r.request)
        if (item && typeof item.price === 'number') {
            revenueByUser[r.user_name] =
                (revenueByUser[r.user_name] || 0) + item.price
        }
    })
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    ...
                                    <div>
                                        نسبة العملاء الذين زاروا مرتين أو أكثر
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                    ...
                    <TabsContent value='financial'>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                                ...
                                <CardContent>
                                    ...
                                    <div className='flex justify-between mt-4'>
                                        <span className='font-bold text-green-700'>
                                            نقدي: {cashRevenue.toLocaleString()}{' '}
                                            جنيه
                                        </span>
                                        <span className='font-bold text-orange-700'>
                                            نقاط الولاء:{' '}
                                            {loyaltyRevenue.toLocaleString()}{' '}
                                            جنيه
                                        </span>
                                    </div>
                                </CardContent>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className='text-3xl font-bold text-green-700 mb-2'>
                                        {cashRevenue.toLocaleString()} جنيه
                                    </div>
                                    <div>
                                        إجمالي الدخل من الطلبات المدفوعة نقدًا
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className='text-3xl font-bold text-orange-700 mb-2'>
                                        {loyaltyRevenue.toLocaleString()} جنيه
                                    </div>
                                    <div>
                                        إجمالي الدخل من الطلبات المدفوعة بنقاط
                                        الولاء
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle>
                                        مقارنة عدد الطلبات والإيرادات اليومية
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <ResponsiveContainer
                    ...
                    <TabsContent value='loyalty'>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className='text-3xl font-bold text-kian-gold mb-4'>
                                        {totalPoints}
                                    </div>
                                    <ResponsiveContainer
                                        width='100%'
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    ...
                                    <div className='flex justify-between mt-4'>
                                        <span className='font-bold text-green-700'>
                                            مشتريات بنقاط الولاء:{' '}
                                            {loyaltyPurchases}
                                        </span>
                                        <span className='font-bold text-orange-700'>
                                            مشتريات نقدية: {cashPurchases}
                                        </span>
                                    </div>
                                </CardContent>
                            </Card>
